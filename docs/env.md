...
| **`auto`** | Train GPU → tương đương `pytorch`; train CPU → `null` |
...
Project NER hiện tại dùng kiến trúc Thinc mặc định (tok2vec + ner), **không bắt buộc** `pytorch` để chạy GPU — nhưng khi bạn cài PyTorch+CUDA cho train GPU, nên để **`pytorch`**.
...
Train GPU thật cần thêm trong image: PyTorch+CUDA (nếu `NER_GPU_ALLOCATOR=pytorch`).
